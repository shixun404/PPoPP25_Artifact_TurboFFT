...
void test_turbofft( DataType* input_d, DataType* output_d, DataType* output_turbofft,
    ...
    printf("turboFFT, %d, %d, %8.3f, %8.3f, %8.3f\n",  (int)log2f(N),  (int)bs, elapsed_time, gflops, mem_bandwidth);
...
void TurboFFT_main(ProgramConfig &config){
    ...
    if(!config.if_bench){
        ...
        if(config.if_profile){
            ...
            profiler::cufft::test_cufft<DataType>(input_d, output_d, output_cufft, config.N, bs, ntest);
            
            for(int bs = bs_begin; bs <= config.bs_end; bs += config.bs_gap)
            test_turbofft<DataType, if_ft, if_err, gpu_spec>(input_d, output_d, output_turbofft, twiddle_d, checksum_d, params[config.logN], bs, config.thread_bs, ntest, config);
        }
    }
