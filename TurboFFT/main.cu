...
template <typename DataType, int if_thread_ft, int if_ft, int if_err, int gpu_spec>
void test_turbofft( DataType* input_d, DataType* output_d, DataType* output_turbofft,
    ...
    TurboFFT_Kernel_Entry<DataType, if_thread_ft, if_ft, if_err, gpu_spec> entry;
    int M = 16;
...
void TurboFFT_main(ProgramConfig &config){
    ...
    if(!config.if_bench){
        ...
        if(config.if_verify){
            test_turbofft<DataType, if_thread_ft, if_ft, if_err, gpu_spec>(input_d, output_d, output_turbofft, twiddle_d, checksum_d, params[config.logN], config.bs, config.thread_bs, 1, config);
        ...
        if(config.if_profile){
            ...
            for(int bs = bs_begin; bs <= config.bs_end; bs += config.bs_gap)
            test_turbofft<DataType, if_thread_ft, if_ft, if_err, gpu_spec>(input_d, output_d, output_turbofft, twiddle_d, checksum_d, params[config.logN], config.bs, config.thread_bs, ntest, config);
        }
    }
...
int main(int argc, char *argv[]){
    ...
    if(config.gpu == "T4"){
        if(config.datatype == 0) {
            if(config.if_ft == 0 && config.if_thread_ft == 0) TurboFFT_main<float2, 0, 0, 0, 75>(config);
            else if(config.if_ft == 0 && config.if_thread_ft == 1) TurboFFT_main<float2, 1, 0, 0, 75>(config);
            else if(config.if_err == 0) TurboFFT_main<float2, 0, 1, 0, 75>(config);
            else TurboFFT_main<float2, 0, 1, 1, 75>(config);
        }
        else {
            if(config.if_ft == 0 && config.if_thread_ft == 0) TurboFFT_main<double2, 0, 0, 0, 75>(config);
            else if(config.if_ft == 0 && config.if_thread_ft == 1) TurboFFT_main<double2, 1, 0, 0, 75>(config);
            else if(config.if_err == 0) TurboFFT_main<double2, 0, 1, 0, 75>(config);
            else TurboFFT_main<double2, 0, 1, 1, 75>(config);
        }
    } else {
        if(config.datatype == 0) {
            if(config.if_ft == 0 && config.if_thread_ft == 0) TurboFFT_main<float2, 0, 0, 0, 80>(config);
            else if(config.if_ft == 0 && config.if_thread_ft == 1) TurboFFT_main<float2, 1, 0, 0, 80>(config);
            else if(config.if_err == 0) TurboFFT_main<float2, 0, 1, 0, 80>(config);
            else TurboFFT_main<float2, 0, 1, 1, 80>(config);
        }
        else {
            if(config.if_ft == 0 && config.if_thread_ft == 0) TurboFFT_main<double2, 0, 0, 0, 80>(config);
            else if(config.if_ft == 0 && config.if_thread_ft == 1) TurboFFT_main<double2, 1, 0, 0, 80>(config);
            else if(config.if_err == 0) TurboFFT_main<double2, 0, 1, 0, 80>(config);
            else TurboFFT_main<double2, 0, 1, 1, 80>(config);
        }
